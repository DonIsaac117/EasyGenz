...
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `aprendiz_ingreso_salida` () BEGIN
SELECT usuarios.nombres as nombre_aprendiz,
  usuarios.apellidos as apellido_aprendiz,
  usuarios.numero_documento,
  numero_ficha.codigo as codigo_ficha,
...
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `fichas` () BEGIN
SELECT numero_ficha.codigo as codigo_ficha,
  numero_ficha.nombre as nombre_ficha,
  numero_ficha.jornada,
...
FROM numero_ficha
  INNER JOIN programa on programa.id = numero_ficha.id_programa
  INNER JOIN tipo_programa on tipo_programa.id = programa.id_tipo_programa;
end $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `funcionarios` () BEGIN
...
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `instructor_ficha` () BEGIN
SELECT usuarios.nombres as nombre_instructor,
...
FROM usuarios
  ...
  INNER JOIN programa on programa.id = numero_ficha.id_programa
  INNER JOIN tipo_programa on tipo_programa.id = programa.id_tipo_programa;
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `instructor_ingreso_salida` () BEGIN
SELECT usuarios.nombres as nombre_aprendiz,
  usuarios.apellidos as apellido_aprendiz,
  usuarios.numero_documento,
  numero_ficha.codigo as codigo_ficha,
...
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `perfiles` () BEGIN
SELECT usuarios.nombres,
  usuarios.apellidos,
  usuarios.numero_documento,
  perfil.perfil
...
END $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `usuarios_ficha` () BEGIN
SELECT usuarios.nombres as nombre,
...
FROM usuarios
  ...
  INNER JOIN numero_ficha on numero_ficha.codigo = aprendiz.codigo_numeroficha
  or numero_ficha.codigo = instructor.codigo_numeroficha
  INNER JOIN programa on programa.id = numero_ficha.id_programa
  INNER JOIN tipo_programa on tipo_programa.id = programa.id_tipo_programa;
end $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `usuario_controlfuncionario` () BEGIN
...
end $$ CREATE DEFINER = `root` @`localhost` PROCEDURE `usuario_ingreso_salida` () BEGIN
SELECT usuarios.nombres,
  usuarios.apellidos,
  usuarios.numero_documento,
  numero_ficha.codigo as codigo_ficha,
...
INSERT INTO `aprendiz` (`id_usuario`, `codigo_numeroficha`)
VALUES (1, 2620031),
  (2, 2620031),
  (3, 42049482),
  (4, 7842958),
...
VALUES (
    ...
    'a la comida',
    NULL,
    'Se ha ingresado un nuevo usuario por parte de :root@localhost'
  ),
  (
    2,
    'root@localhost',
    ...
    '',
    'Gluten',
    'Se ha Eliminado un usuario por parte de :root@localhost'
  );
-- --------------------------------------------------------
--
...
INSERT INTO `controlfuncionarios` (
    `id_usuario`,
    `fecha`,
    `hora_entrada`,
    `hora_salida`,
...
INSERT INTO `funcion` (`id`, `nombre`, `descripcion`)
VALUES (
    1,
    'Coordinador de programas',
    'Coordinar la implementación de programas de formación y capacitación.'
  ),
  (
    2,
    'Asesor en orientación laboral',
    'Brindar asesoramiento y orientación a los estudiantes y graduados sobre opciones de carrera y oportunidades laborales.'
  ),
  (
    3,
    'Gestor de proyectos',
    'Planificar, coordinar y supervisar la ejecución de proyectos relacionados con la formación y el desarrollo profesional.'
  ),
  (
    4,
    'Analista de recursos humanos',
    'Administrar procesos de reclutamiento y selección de personal para vacantes internas y externas.'
  ),
  (
    5,
    'Coordinador de prácticas empresariales',
    'Establecer y mantener relaciones con empresas y organizaciones para facilitar oportunidades de prácticas laborales para los estudiantes.'
  );
-- --------------------------------------------------------
--
...
INSERT INTO `funcionario` (`id_usuario`, `id_funcion`)
VALUES (9, 1),
  (9, 2);
-- --------------------------------------------------------
--
...
INSERT INTO `ingresosalida_ficha` (
    `id_usuario`,
    `codigo_numeroficha`,
    `fecha`,
    `hora_entrada`,
...
INSERT INTO `instructor` (`id_usuario`, `codigo_numeroficha`)
VALUES (6, 2620031),
  (7, 42049482),
  (8, 7842958);
-- --------------------------------------------------------
...
INSERT INTO `programa` (`id`, `nombre`, `id_tipo_programa`)
VALUES (398244, 'ADSO ', 1),
  (3862842, 'Sistemas', 2),
...
INSERT INTO `tipo_programa` (`id`, `tipo`, `descripcion`)
VALUES (
    1,
    'Tecnologo',
    'Duración de la ficha 4 semestres con un semestre de practicas y 3 de lectiva'
  ),
  (
    2,
    'Tecnico',
    'un año con 6 meses de lectiva y 6 de practica'
  ),
  (
    3,
    'curso corto',
    'Oscila entre las 40 y 80 horas'
  );
-- --------------------------------------------------------
--
...
VALUES (
  ...
  (
    ...
    'AB+',
    2147483647,
    'patear el valon peor que un burro',
    'todas'
  ),
  (
    5,
...
values (
    ...
    old.alergias,
    concat(
      "Se ha modificado un usuario por parte de :",
      user()
    )
  );
END $$ DELIMITER;
DELIMITER $$ CREATE TRIGGER `eliminar_usuario`
...
values (
    ...
    old.alergias,
    concat(
      "Se ha Eliminado un usuario por parte de :",
      user()
    )
  );
END $$ DELIMITER;
DELIMITER $$ CREATE TRIGGER `eliminar_usuario_total` BEFORE DELETE ON `usuarios` FOR EACH ROW BEGIN -- Eliminar registros relacionados en la tabla 'ingresosalida_ficha'
DELETE FROM ingresosalida_ficha
...
DELETE FROM controlfuncionarios
WHERE id_usuario = OLD.id;
...
WHERE id_usuario = OLD.id;
END $$ DELIMITER;
DELIMITER $$ CREATE TRIGGER `ingreso_nuevo_usuario`
AFTER
...
values (
    ...
    new.alergias,
    concat(
      "Se ha ingresado un nuevo usuario por parte de :",
      user()
    )
  );
END $$ DELIMITER;
